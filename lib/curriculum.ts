...
export const RESOURCES: [string, string, string, string, string][] = [
  ...
  ["Full Stack Open", "https://fullstackopen.com/en/", "course", "Full Stack", "University of Helsinki's free, exercise-driven full stack course. Parts 0 to 5, 9, 11 to 14 are linked from the Engineering tasks they teach, scoped by chapter (the later parts now live on courses.mooc.fi); parts 6, 7, 8 and 10 are optional. Never a completion target"],
...
// Full Stack Open (University of Helsinki), the exercise-driven spine of
// the Engineering track. Only the parts the roadmap actually links appear
// here; the test suite pins this set. Verified 2026-09-15: parts 0 to 5
// live on fullstackopen.com with lettered chapters (3a, 3b); parts 9 and
// 11 to 14 moved to courses.mooc.fi with numbered chapters, where chapter
// 1 is always "Getting started". Parts 6 (Redux / React Query), 7 (custom
// hooks, esbuild), 8 (GraphQL) and 10 (React Native) are deliberately
// optional. Links go to the part's index page (chapters are named in the
// task description) so a renamed chapter never rots a link.
export const FULL_STACK_OPEN_PARTS: Record<number, { title: string; url: string }> = {
  0: { title: "Fundamentals of Web apps", url: "https://fullstackopen.com/en/part0" },
  1: { title: "Introduction to React", url: "https://fullstackopen.com/en/part1" },
  2: { title: "Communicating with server", url: "https://fullstackopen.com/en/part2" },
  3: { title: "Programming a server with NodeJS and Express", url: "https://fullstackopen.com/en/part3" },
  4: { title: "Testing Express servers, user administration", url: "https://fullstackopen.com/en/part4" },
  5: { title: "Testing React apps, React Router", url: "https://fullstackopen.com/en/part5" },
  9: { title: "TypeScript", url: "https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-typescript" },
  11: { title: "CI/CD", url: "https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-continuous-integration" },
  12: { title: "Containers", url: "https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-containers" },
  13: { title: "Using relational databases", url: "https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-relational-databases" },
  14: { title: "Next.JS", url: "https://courses.mooc.fi/org/uh-cs/courses/full-stack-open-nextjs" },
};
const fso = (part: number) => ({
  label: `Full Stack Open part ${part}: ${FULL_STACK_OPEN_PARTS[part].title}`,
  url: FULL_STACK_OPEN_PARTS[part].url,
});
...
export const WEEK_TASKS: Record<number, SeedTask[]> = {
  1: [
    ...
    { day: 1, title: "How the web works", category: "Engineering", estMinutes: 45, description: "Read Full Stack Open 0b with DevTools open: reproduce its request diagrams for the example app in the Network tab, then draw the browser-to-server sequence for a form submit from memory. Every server session from here assumes this picture.", links: [fso(0)] },
    ...
    { day: 2, title: "React fundamentals", category: "Engineering", estMinutes: 120, description: "Pick one path and finish it: work react.dev Learn from Describing the UI through Adding Interactivity doing every challenge, or do Full Stack Open 1a through 1d submitting the exercises as you go, or code along with the React 19 course video (2 hr). Same ground each way; do not do two today.", links: [{ label: "react.dev Learn (do the challenges)", url: "https://react.dev/learn" }, fso(1), { label: "React 19 Course (video, 2 hr)", url: "https://www.youtube.com/watch?v=dCLhUialKPQ" }] },
    ...
    { day: 3, title: "TypeScript fundamentals", category: "Engineering", estMinutes: 120, description: "Work the Beginner's TypeScript exercises in order, attempting each one before revealing the solution. When a compiler error stumps you, look it up in the handbook before moving on. Full Stack Open part 9 chapters 2 and 3 are the same ground in longer form; keep chapters 4 and 5 (typing Express and React) for when you write that code in weeks 2 and 3.", links: [{ label: "Beginner's TypeScript (interactive)", url: "https://www.totaltypescript.com/tutorials/beginners-typescript" }, { label: "TS Handbook", url: "https://www.typescriptlang.org/docs/handbook/intro.html" }, fso(9)] },
  ...
  2: [
    ...
    { day: 1, title: "React depth", category: "Engineering", estMinutes: 90, description: "Work Managing State's challenges today (about 90 minutes); finish the Escape Hatches challenges across the week. Then do Full Stack Open 2b through 2d across the week too: forms, fetching from a server, effects, error handling. Those exercises are exactly the muscle Life Companion's screens need. The challenges are the learning; reading alone does not count.", links: [{ label: "react.dev: Managing State", url: "https://react.dev/learn/managing-state" }, { label: "react.dev: Escape Hatches", url: "https://react.dev/learn/escape-hatches" }, fso(2)] },
    ...
    { day: 2, title: "Next.js fundamentals", category: "Engineering", estMinutes: 90, description: "Work the official interactive course through routing, layouts, and data fetching today (about 90 minutes), building the demo as you go; finish the remaining chapters across the week. Full Stack Open part 14 chapter 2 covers the same App Router ground from the server side; read it when the official course leaves a question open. The JS Mastery video is an optional deeper build, not for today.", links: [{ label: "Next.js Learn (interactive)", url: "https://nextjs.org/learn" }, fso(14), { label: "JSM Next.js 15 course (video, 7 hr, optional)", url: "https://www.youtube.com/watch?v=3qGQ4dGA42o" }] },
  ...
  3: [
    ...
    { day: 1, title: "PostgreSQL setup and basics", category: "Engineering", estMinutes: 90, description: "Install Postgres (or run it in Docker), then follow the Prisma quickstart until you can create and query rows from TypeScript. Finish by writing two queries by hand in psql; this is the Life Companion stack. Full Stack Open part 13 chapters 2 and 3 cover the same relational ideas with Sequelize; read them if Prisma's abstractions hide what a query or a join actually does.", links: [{ label: "Prisma + Postgres quickstart", url: "https://www.prisma.io/docs/getting-started/setup-prisma/start-from-scratch/relational-databases-typescript-postgresql" }, { label: "PostgreSQL tutorial", url: "https://www.postgresql.org/docs/current/tutorial.html" }, fso(13)] },
    ...
    { day: 4, title: "Authentication", category: "Engineering", estMinutes: 120, description: "Read The Copenhagen Book's sessions and password chapters (about 45 minutes), then wire Supabase Auth into Life Companion: sign-up, sign-in, one protected page. Full Stack Open 4c and 4d (user administration, token authentication) show what Supabase is doing for you; read them so you can explain hashing and tokens in an interview.", links: [{ label: "The Copenhagen Book", url: "https://thecopenhagenbook.com" }, { label: "Supabase Auth docs", url: "https://supabase.com/docs/guides/auth" }, fso(4)] },
    { day: 5, title: "API design and building", category: "Engineering", estMinutes: 120, description: "Work Full Stack Open 3a and 3b first (about 60 minutes): Express routes, middleware, error handling, deployment. Skip 3c and 3d; they use MongoDB and Life Companion is Postgres. Then design first (resources, verbs, status codes, errors) and build a couple of Life Companion's real endpoints with Next.js route handlers.", links: [fso(3), helloCore, { label: "Next.js: Route Handlers", url: "https://nextjs.org/docs/app/building-your-application/routing/route-handlers" }] },
  ...
  4: [
    ...
    { day: 2, title: "Testing", category: "Engineering", estMinutes: 120, description: "Work Full Stack Open 5c (component tests with Vitest and Testing Library) and 4b (API tests with supertest), about 75 minutes together. Then write real tests against Life Companion code, not toy examples. 5d is Playwright end-to-end testing; do it when a full flow is worth guarding.", links: [fso(5), fso(4), { label: "Vitest guide", url: "https://vitest.dev/guide/" }, { label: "Testing Library: queries", url: "https://testing-library.com/docs/queries/about/" }] },
    patternStudy(2, "Graph BFS / DFS pattern"),
    { day: 3, title: "CI/CD", category: "Engineering", estMinutes: 90, description: "Work Full Stack Open part 11 chapters 2 through 4 (about 60 minutes): what CI is for, a GitHub Actions workflow, deploying from it. Then wire a real workflow on Life Companion: lint + tests on every push. The quickstart is the reference when the syntax stumps you.", links: [fso(11), { label: "GitHub Actions quickstart", url: "https://docs.github.com/en/actions/quickstart" }] },
    patternStudy(3, "Matrix / islands pattern"),
    { day: 4, title: "Docker", category: "Engineering", estMinutes: 120, description: "Do Full Stack Open part 12 chapters 2 and 3 (about 75 minutes): images, containers, a Node app in a container, Compose. Then write a Dockerfile for Life Companion yourself. You are done when docker compose up serves the app locally. Docker's own Get Started is the reference.", links: [fso(12), { label: "Docker Get Started", url: "https://docs.docker.com/get-started/" }] },
